...
from polysolver_naive_theo import naive_approach_theo
import sys
...
def Writer(challenge: str, method: str) -> None:
    ...
    output_name = (
        "solutions/" + str(str(challenge.split("/")[-1]).split(".")[0]) + ".out"
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("usage: python polywriter.py naive_loic|naive_theo challenges/ma_map.in")
    else:
